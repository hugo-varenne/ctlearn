...
def get_tel_pointing(file, tel_id) -> Table:
    """
...
def get_array_pointing(file) -> Table:
    """
...
def _transform_to_log_energy(table):
    """
...
def _transform_to_impact_radius(table) -> Table:
    """
...
def _transform_to_cam_coord_offsets(subarray, pix_rotation, table) -> Table:
    """
    ...
    tel_ground_frame = subarray.tel_coords[
        subarray.tel_ids_to_indices(tel_id)
    ]
    ...
    camera_frame = CameraFrame(
        focal_length=subarray.tel[tel_id].camera.geometry.frame.focal_length,
        rotation=pix_rotation[tel_id],
...
def _transform_to_sky_spher_offsets(subarray, table) -> Table:
    """
...
def _parse_subarray_metadata(filename, focal_length_choice, selected_tel_ids):
    # Read SubarrayDescription from the new file
    subarray = SubarrayDescription.from_hdf(filename, focal_length_choice=focal_length_choice)
    # Filter subarray by selected telescopes
    if selected_tel_ids is not None:
        subarray = subarray.select_subarray(selected_tel_ids)
    return filename, subarray

def process_file(constant_args, file_info):
            tel_type, selected_telescopes, process_type, force_dl1_lookup, subarray, pix_rotation, quality_query, example_ids_keep_columns, input_url_signal = constant_args
            file_idx, filename = file_info
            _, f = _get_file(filename)
            simulation_info_file= None
            # Read the trigger table.
            trigger_table = read_table(f, "/dl1/event/subarray/trigger")
            if process_type == ProcessType.Simulation:
                # Read simulation information for each observation
                simulation_info_file = read_table(f, "/configuration/simulation/run")
            ...
            for tel_id in selected_telescopes[tel_type]:
                ...
                if process_type == ProcessType.Simulation:
                    ...
                    tel_pointing = get_tel_pointing(f, tel_id)
                    tel_table = join(
                        left=tel_table,
                        right=tel_pointing,
                        keys=["obs_id", "tel_id"],
                    )
                    tel_table = _transform_to_cam_coord_offsets(subarray, pix_rotation, tel_table)
            ...
            passes_quality_checks = np.ones(len(events), dtype=bool)
            # Quality selection based on the dl1b parameter and MC shower simulation tables
            if quality_query:
                passes_quality_checks = quality_query.get_table_mask(events)

            # Apply the mask to filter events that are not fufilling the quality criteria
            events = events[passes_quality_checks]

            # Construct the example identifiers
            events.keep_columns(example_ids_keep_columns)
            if process_type == ProcessType.Simulation:
                # Add the spherical offsets w.r.t. to the telescope pointing
                array_pointing = get_array_pointing(f)
                ...
                events = _transform_to_sky_spher_offsets(subarray, events)
                # Add the logarithm of the true energy in TeV
                events = _transform_to_log_energy(events)
                # Add the impact radius
                events = _transform_to_impact_radius(events)
                ...
                true_shower_primary_class = (
                    1 if filename in input_url_signal else 0
                )
                events.add_column(
            ...
            return (simulation_info_file, events)
    ...
    **kwargs : dict
    ...
    def __init__(
        ...
        **kwargs,
        ...
        sorted_file_list = np.sort(file_list)
        self.files = OrderedDict((f, None) for f in sorted_file_list)
        self.log.info(f"Loading {len(sorted_file_list)} files with multiprocessing...")
        self.log.info(f"CPU available : {cpu_count()}")
        with ThreadPoolExecutor(max_workers=(min(8, cpu_count()))) as pool:
            for file_path, result in pool.map(_get_file, sorted_file_list):
                if isinstance(result, Exception):
                    raise RuntimeError(f"Error loading {file_path}: {result}")
                self.files[file_path] = result
        self.log.info("All files loaded.")
        ...
        self.log.info("Get metadata")
        with Pool(min(8, cpu_count())) as pool:
            results = pool.map(partial(_parse_subarray_metadata, focal_length_choice=self.focal_length_choice, selected_tel_ids=selected_tel_ids), list(self.files.keys()))

        for filename, subarray in results:
    ...
    def _construct_mono_example_identifiers(self):
        ...
        constant_args = (self.tel_type, self.selected_telescopes, self.process_type, self.force_dl1_lookup, self.subarray, self.pix_rotation, self.quality_query, self.example_ids_keep_columns, self.input_url_signal)
        
        file_list = []
        for idx, filename in enumerate(self.files.keys()):
        ...
        # Extract results in separated files
        unstacked_identifiers, unstacked_events = [], []
        for (sim_info, ev) in results:
            unstacked_identifiers.append(sim_info)
            unstacked_events.append(ev)
            
        # Construct the example identifiers for all files       
        example_identifiers = vstack(unstacked_events)
        ...
        if self.process_type == ProcessType.Simulation:
            self.simulation_info = vstack(unstacked_identifiers)
