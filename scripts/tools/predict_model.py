...
# Stream handler -> ensures logs go to stdout (SLURM .out file)
...
# Make sure unhandled exceptions are also logged
...
logging.info("Logging initialized. All stdout/stderr will go to SLURM log.")
...
class PredictCTLearnModel(Tool):
    ...
    def _predict_with_model(self, model_path):
        ...
        prediction_colname = (
            "type" if isinstance(model.layers[-1], keras.layers.Softmax) or "softmax" in model.layers[-1].name else model.layers[-1].name
        )
