...
class TrainCTLearnModel(Tool):
    ...
    pruning_model = Dict(
        ...
        allow_none=True,
    	help=(
    	    "Pruning parameters to train a model. "
    	    "E.g. {'initial_sparsity': 0.00, 'final_sparsity' : 0.30, 'begin_step': 0}. "
    	)
    ).tag(config=True)
    ...
    percentage_per_epoch = Float(
        default_value=1.0,
    	help=(
    	    "Percentage of batches used per epoch"
            "Helpful to have faster epochs"
    	),
        min=0.001,
        max=1.0
    ).tag(config=True)
    ...
    def setup(self):
        ...
        if self.output_dir.exists():
            ...
            if os.path.exists(interrupt_path):
                ...
                self.rerun = True
                # Create path to interrupted model
                if int(keras.__version__.split(".")[0]) >= 3:
                    self.interrupted_model = f"{self.output_dir}/interrupt/model.keras"
                else:
                    self.interrupted_model = f"{self.output_dir}/interrupt/model.cpk"
            else:
        ...
        if int(keras.__version__.split(".")[0]) >= 3:
            ...
            interrupt_path = f"{self.output_dir}/interrupt/model.keras"
        else:
            model_path = f"{self.output_dir}/ctlearn_model.cpk"
            interrupt_path = f"{self.output_dir}/interrupt/model.cpk"
        ...
        self.callbacks = [model_checkpoint_callback, latest_checkpoint_callback, tensorboard_callback, csv_logger_callback]
    ...
    def start(self):   
        ...
        with self.strategy.scope():
            # Check if model currently in training 
            if self.rerun:
                self.log.info("Resuming training from previous run")
                training_csv = os.path.join(self.output_dir, "training_log.csv")
                if os.path.exists(training_csv):
                    df = pd.read_csv(training_csv)
                    self.interrupted_epoch = len(df)
                # Load the model
                self.model = keras.models.load_model(self.interrupted_model)
            else:
                # Construct the model
            ...
            # Set steps per epochs (number of batches used at each epochs)
            self.steps_per_epoch = max(1, int(len(self.training_loader) * self.percentage_per_epoch))
            if not self.rerun:
                ...
                if self.pruning_model is not None:
                    ...
                    self.log.info("Parameters for pruning: initial_sparsity=%s, final_sparsity=%s, begin_step=%s", 
                                 self.pruning_model["initial_sparsity"], self.pruning_model["final_sparsity"], self.pruning_model["begin_step"])
                    pruning_schedule = tfmot.sparsity.keras.PolynomialDecay(initial_sparsity=self.pruning_model["initial_sparsity"],
                                                         final_sparsity=self.pruning_model["final_sparsity"],
                                                         begin_step=self.pruning_model["begin_step"],
                                                         end_step=end_step)
                    # Apply pruning to the model
                    self.model = prune_low_magnitude(self.model, pruning_schedule=pruning_schedule)
    ...
    def finish(self):
        ...
        self.log.info("Remove interrupted directory")
        if os.path.exists(os.path.join(self.output_dir, "interrupt")):
            shutil.rmtree(os.path.join(self.output_dir, "interrupt"))
